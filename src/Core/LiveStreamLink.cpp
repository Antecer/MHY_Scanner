...
namespace
{
constexpr const char* kBiliWebUserAgent =
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
    "(KHTML, like Gecko) Chrome/126.0 Safari/537.36";

constexpr const char* kDouyinWebUserAgent =
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
    "(KHTML, like Gecko) Chrome/126.0 Safari/537.36";
}
...
LiveStreamInfo LiveDouyin::GetLiveStreamInfo()
{
    ...
    try
    {
        // 构建请求参数
        const cpr::Header headers = {
            { "User-Agent", kDouyinWebUserAgent },
...
std::map<std::string, std::string> GetLiveStreamOpenOptions(const LivePlatform platform)
{
    switch (platform)
    {
    case LivePlatform::BiliBili:
        return {
            { "user_agent", kBiliWebUserAgent },
            { "headers", "Referer: https://live.bilibili.com/\r\nOrigin: https://live.bilibili.com\r\n" }
        };
    case LivePlatform::Douyin:
        return {
            { "user_agent", kDouyinWebUserAgent },
